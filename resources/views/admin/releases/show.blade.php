            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                    ...
                    <p class="text-slate-500 text-sm">Judul</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $release->title }}</h3>
                    </div>
                    <div class="flex flex-col">
                        @if($release->slider)
                ...
                <div class="flex flex-row justify-between items-center align-middle">
                    <div>
                        ...
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $release->user->name }}</h3>
                    </div>
                    <div>
                        <p class="text-slate-500 text-sm">Tanggal Dibuat</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $release->created_at->format('d M Y') }}</h3>
                    </div><div>
                        <p class="text-slate-500 text-sm">Cover</p>
                        <img src="{{ $release->cover }}" alt="{{ $release->title }}" class="rounded-2xl object-cover w-[120px] h-[90px]">
                    </div>
                </div>
                ...
                <div class="flex flex-col">
                        <div>
                            ...
                            <p class="text-slate-500 text-m">{{ $release->summary }}</p>
                        </div> 
                </div>
                <div class="flex flex-col">
                        <div>
                            ...
                            <br>
                            <p class="text-slate-500 text-m">{!! $release->content !!}</p>
                        </div> 
                </div>

                <hr class="my-2">
                <div class="flex flex-row items-center gap-x-3">
                        <a href="{{ route('admin.releases.edit', $release) }}" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                            Edit
                        </a>
