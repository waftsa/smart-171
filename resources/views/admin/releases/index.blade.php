            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10">
                ...
                <div class="flex justify-between items-center mb-6">
                    <div class="flex items-center gap-4">
                        <form action="{{ route('admin.releases.index') }}" method="GET" class="flex flex-wrap gap-4 items-center">
                    ...
                    <a href="{{ route('admin.releases.create') }}" class="font-bold py-2 px-4 bg-white border-2 border-green-700 text-green-700 rounded-lg hover:bg-green-300 hover:border-green-300">
                        Add New
                    </a>
                </div>
                ...
                <table class="min-w-full table-auto">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b">
                            <td class="px-4 py-2">
                                <img src="{{ $release->cover }}" alt="{{ $release->title }}" class="w-[120px] h-[90px] object-cover rounded-lg">
                            </td>
                            <td class="px-4 py-2">
                                {{ $release->title }}
                            </td>
                            <td class="px-4 py-2">
                                {{ $release->updated_at->format('d M Y') }}
                            </td>
                            <td class="px-4 py-2">
                                @if($release->status)
                            ...
                            <td class="px-4 py-8 flex gap-3">
                                <a href="{{ route('admin.releases.edit', $release) }}" class="text-yellow-500 mt-2">
                                ...
                                <form action="{{ route('admin.releases.destroy', $release) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus artikel ini?');">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <a href="{{ route('admin.releases.show', $release) }}" class="px-4 py-2 bg-indigo-700 text-white rounded-lg font-bold">
                                    Detail
                                </a>
                            </td>
                        </tr>
                ...
                <div class="mt-5">
                    {{ $releases->links() }} <!-- Pagination Links -->
                </div>
            </div>
        </div>
    </div>
