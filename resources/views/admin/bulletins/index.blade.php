            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10">
                ...
                <div class="flex justify-between items-center mb-6">
                    <div class="flex items-center gap-4">
                        <form action="{{ route('admin.bulletins.index') }}" method="GET" class="flex flex-wrap gap-4 items-center">
                    ...
                    <a href="{{ route('admin.bulletins.create') }}" class="font-bold py-2 px-4 bg-white border-2 border-green-700 text-green-700 rounded-lg hover:bg-green-300 hover:border-green-300">
                        Add New
                    </a>
                </div>
                ...
                <table class="min-w-full table-auto">
                    ...
                    <tbody>
                        @forelse($bulletins as $buletin)
                        <tr class="border-b">
                            <td class="px-4 py-2">
                                <canvas
                                    id="pdf-{{ $buletin->id }}"
                                    data-pdf="{{ asset('storage/' . $buletin->file) }}"
                            ...
                            <td class="px-4 py-2">
                                {{ $buletin->updated_at->format('d M Y') }}
                            </td>
                            <!-- <td class="px-4 py-2">
                                @if($buletin->status)
                            ...
                            <td class="px-4 py-8 flex gap-3">
                                <!-- <a href="{{ route('admin.bulletins.edit', $buletin) }}" class="text-yellow-500 mt-2">
                                ...
                                <form action="{{ route('admin.bulletins.destroy', $buletin) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus artikel ini?');">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <a href="{{ route('admin.bulletins.show', $buletin) }}" class="px-4 py-2 bg-indigo-700 text-white rounded-lg font-bold">
                                    Detail
                                </a>
                            </td>
                        </tr>
                ...
                <div class="mt-5">
                    {{ $bulletins->links() }} <!-- Pagination Links -->
                </div>
            </div>
        </div>
    </div>
