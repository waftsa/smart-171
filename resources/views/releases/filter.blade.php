    ...
    <div class="py-8 bg-white">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center m-10">
                <a href="{{ route ('releases.list') }}">
                <h2 class="text-indigo-950 text-3xl font-bold">SMART NEWS</h2>
                </a>
                <div class="w-full sm:w-1/3 lg:w-1/4">
                    <!-- Search Bar -->
                    <form action="{{ route('releases.filter') }}" method="GET" class="flex items-center">
            ...
            <div class="flex justify-start gap-4 m-10">
                <a href="{{ route('releases.filter', ['sort_by' => 'newest']) }}" 
                ...
                <a href="{{ route('releases.filter', ['sort_by' => 'oldest']) }}" 
                ...
                <a href="{{ route('releases.filter', ['sort_by' => $sortBy, 'category' => $category->id]) }}" 
                    class="px-4 py-2 bg-white text-[#1D4161] border border-[#1D4161] rounded-lg hover:bg-[#1D4161] hover:text-white hover:border-[#1D4161]">
                    {{ $category->name }}
                </a>
                @endforeach
                        ...
                        <div class="mb-4">
                            <a href="{{ route('releases.show', $release) }}">
                                <div class="relative">
                                    ...
                                    <img src="{{ $release->cover }}" alt="{{ $release->title }}" class="w-full md:h-28 ms:h-24 lg:h-48 object-cover rounded-md mb-3">
                                    ...
                                    <span class="absolute top-2 left-2 bg-white text-[#1D4161] text-xs font-semibold px-2 py-1 rounded-xl">
                                        {{ $release->category?->name ?? 'Tanpa Kategori' }}
                                    </span>
                                </div>

                                <h3 class="font-semibold text-xl text-semibold">{{ $release->title }}</h3>
                                <p class="text-gray-500 text-sm">By
                                    <span class="inline-block text-[#e16976]">{{ $release->user->name }}</span>
